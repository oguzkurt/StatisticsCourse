...
\begin{prompt}
$$X\sim Binomial(100,0.99)\Longrightarrow\Pr(X\leq 3)=\answer[tolerance=0.0000001]{\sage{binomial(100,0)*(0.99)^100*(0.01)^0+binomial(100,1)*(0.99)^99*(0.01)^1+binomial(100,2)*(0.99)^98*(0.01)^2+binomial(100,3)*(0.99)^97*(0.01)^3}}$$
\end{prompt}
...
\begin{prompt}
$$Y\sim Geometric(0.1)\Longrightarrow\Pr(Y\leq 3)=\answer[tolerance=0.001]{\sage{0.1+0.1*0.9+0.1*0.9^2}}$$
\end{prompt}
...
\begin{prompt}    
    ...
    $$\Pr(Y\leq 3)=\answer[tolerance=0.000001]{\sage{exp(-25)*(1+25+25^2/2+25^3/6)}}$$
\end{prompt}
...
\begin{prompt}    
    ...
    $$\Pr(W\geq 3)=1-\Pr(W=2)-\Pr(W=1)-\Pr(W=2)=\answer[tolerance=0.00001]{\sage{1-exp(-3)*(1+3+3^2/2)}}$$
\end{prompt}    
...
We count the total number of defects in a 5-day interval. So, we need to use a random variable $D=Y+Z$ where $Y\sim Poi(25)$ and $Z\sim Poi(15)$ on the same 5-day interval size. Hence, $D\sim Poi(\answer{25+15})$. The answer then is
    $$\Pr(Z=10)=\answer[tolerance=0.00000001]{\sage{exp(-40)*40^10/factorial(10)}}$$
\end{prompt}    
...
\begin{itemize}
 ...
 $E[X]=\answer{21/6}$ and $Var(X)=\answer[tolerance=0.000001]{\sage{((1-3.5)^2+(2-3.5)^2+(3-3.5)^2+(4-3.5)^2+(5-3.5)^2+(6-3.5)^2)/6}}$
...
\begin{enumerate}
    ...
    \item $\mathbb Var[Y]=\answer[tolerance=0.000001]{\sage{3*((1-3.5)^2+(2-3.5)^2+(3-3.5)^2+(4-3.5)^2+(5-3.5)^2+(6-3.5)^2)/6}}$
