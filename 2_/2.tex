...
\input{../preamble.tex}
\title{Counting and Probability -- Homework 2}
\author{Oguz Kurt}
...
\section*{Syntax}
In this HW, you are expected to solve problems of counting. Here is a reminder of the way to input some numbers. Please, use the parsing power of your browser instead of using a calculator.
...
\begin{problem}
From a group of 30 staff member, 10 will be sent to vacation by a lottery. 

\begin{enumerate}
    \item The number of ways to select the lucky 10 folks is $\answer{\sage{c(30,10)}}$. 
    \item Suppose also that 2 will go to Hawaii, 5 will go to Florida and 3 will go to Quebec. The number  is $\answer{\sage{c(30,10)*m(10,[2,3,5])}}$.
    \item Suppose also that each group will have a team leader who is in charge of fun. The number then is $\answer{\sage{c(30,10)*m(10,[2,3,5])*2*3*5}}$.    
\end{enumerate}
\end{problem}
\begin{problem}
