...
\end{equation*}
Because the trials are independent, the probability of each success is $\answer{\sage{p}}$.
...
\end{sagesilent}
\begin{equation*} 
\Pr(5 \text{ successes in  } k \text{ trials } ) = \answer{\sage{binomial(k,5)*p^5*(1-p)^(k-5)}} 
\end{equation*}
\end{explanation}
